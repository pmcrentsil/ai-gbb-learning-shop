...
load_dotenv()
AZ_FOUNDRY_PROJECT_CONNECTION_STRINGS = os.getenv("AZ_FOUNDRY_PROJECT_CONNECTION_STRINGS")
...
def get_recommendations(project_client, agent, extracted_content):
    ...
    recommendations = project_client.agents.list_messages(thread_id=recommendation_thread.id).get("data", [])
...
def main():
    ...
    recommendations = get_recommendations(project_client, gpt4o_agent, extracted_content)
